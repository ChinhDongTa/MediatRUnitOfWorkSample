...
public class CustomerUowController(IChinookUow uow) : ControllerBase {
    ...
    [HttpGet("GetOne/{id}")]
    public async Task<IActionResult> GetOne(int id) => Ok(ApiResultExtension.GetApiResult(await uow.GetCustomerDtoById(id)));

    [HttpGet("GetAll")]
    public async Task<IActionResult> GetAllAlbum() => Ok(ApiResultExtension.GetApiResult(await uow.GetListCustomerDtos()));
